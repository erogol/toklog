...
class TestLogRawCallSiteCapture:
    def test_log_raw_auto_captures_call_site(self, tmp_path: Path) -> None:
        """log_raw() without caller arg invokes _get_call_site() for auto-detection."""
        import toklog
        from unittest.mock import patch

        fake_cs = {"file": "my_app.py", "function": "process_query", "line": 42}

        with patch("toklog.wrapper._get_call_site", return_value=fake_cs):
            toklog.log_raw(
                provider="openai",
                model="gpt-4o",
                input_tokens=100,
        ...
        assert len(rows) == 1
        row = rows[0]
        assert row["call_site"] == "my_app.py:process_query:42"
        assert row["calls"] == 1

    def test_log_raw_explicit_caller_overrides_auto(self, tmp_path: Path) -> None:
        """log_raw() with caller arg uses the provided value; _get_call_site is not called."""
        import toklog
        from unittest.mock import patch

        # _get_call_site should NOT be called when caller is explicitly provided
        with patch("toklog.wrapper._get_call_site") as mock_cs:
            toklog.log_raw(
                provider="openai",
                model="gpt-4o",
                input_tokens=100,
                output_tokens=20,
                caller="custom/path.py",
            )
        ...
        rows = report["cost_by_call_site"]
        assert len(rows) == 1
        assert "custom/path.py" in rows[0]["call_site"]

    def test_log_raw_call_site_none_when_detection_fails(self, tmp_path: Path) -> None:
        """When _get_call_site() returns None, entry has no call_site and falls back to tag."""
        import toklog
        from unittest.mock import patch

        with patch("toklog.wrapper._get_call_site", return_value=None):
            toklog.log_raw(
                provider="anthropic",
                model="claude-sonnet-4-6",
                input_tokens=200,
        ...
        rows = report["cost_by_call_site"]
        assert rows
        # Falls back to [tag] bucket since no call_site
        assert rows[0]["call_site"] == "[inference]"
